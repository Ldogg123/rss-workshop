...
(() => {
 ...
 const max={keywords:+bounds.dataset.maxKeywords,nodes:+bounds.dataset.maxNodes,depth:+bounds.dataset.maxDepth,characters:+bounds.dataset.maxKeywordCharacters};
 ...
 // One include/exclude tree editor. The feed editor and the filter library
 // each own an instance bound to their own panels.
 function editor({section,include,exclude,count,error,onUpdate}){
  const panels={include,exclude};
  let roots={}, nextID=0;
  ...
  function load(filters){
   ...
   render();return !!(roots.include||roots.exclude);
  }
  function read(){
   ...
   }catch(e){if('open' in section)section.open=true;error.textContent=e.message;error.hidden=false;throw e;}
  }
  return {load,read,update};
 }
 // The feed editor's summary also counts the library filters it uses.
 function summarize(n=feed.totals){
  feed.totals=n;
  const used=window.rssLibrary?.selectedCount()||0;
  const parts=[];if(n.rules)parts.push(plural(n.keywords,'phrase'));if(used)parts.push(plural(used,'library filter'));
  $('#filter-summary-count').textContent=parts.length?parts.join(' · '):'Optional';
 }
 const feed=editor({section:$('#story-filters'),include:$('#filter-include'),exclude:$('#filter-exclude'),count:$('#filter-count'),error:$('#filter-error'),onUpdate:summarize});
 feed.totals={keywords:0,rules:0};
 function load(filters,editing){
  $('#story-filters').open=feed.load(filters);$('#apply-filter-history').hidden=!editing;
  $('#apply-filters-to-history').checked=false;
 }
 ...
 window.rssFilters={load,read:feed.read,preview,editor,summarize,phrases:set=>{let n=0;const visit=rule=>{if(!rule)return;if(isGroup(rule))rule.rules.forEach(visit);else n+=rule.keywords.length;};visit(set?.include);visit(set?.exclude);return n;}};
