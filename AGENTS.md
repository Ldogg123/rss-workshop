...
- Schema 2 adds run diagnostics; schema 3 prevents older apps from silently ignoring filters stored in recipe JSON. Retain every released migration and frozen schema fixture so users can upgrade directly from any released schema without installing intermediate app versions. Test the full transactional chain, existing history preservation and rollback on failure. Keep backup support for schemas 1, 2, and 3; unknown/newer schemas must still be rejected. Downgrades require a backup compatible with the target app version.
