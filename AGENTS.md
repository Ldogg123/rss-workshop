...
- Schema 2 adds run diagnostics; schema 3 prevents older apps from silently ignoring filters stored in recipe JSON. Keep explicit transactional upgrades and released-schema fixtures for schemas 1 and 2, plus backup support for schemas 1, 2, and 3. Downgrades require a backup compatible with the target app version.
...
- GitHub and registry publication use the separate manual release process. Do not turn local development or testing into a release implicitly.
