...
Run a local instance using the [source setup](docs/deployment.md#run-from-source). Native development reads environment variables directly; `.env` is loaded by Compose only. Use a separate data directory and port when another instance is already running. The default Docker Compose installation includes Chromium; see [deployment](docs/deployment.md) for its host-directory setup. `compose.static.yaml` selects the lightweight runtime without local Chromium. Include it before `compose.image.yaml` when using a prebuilt `-static` image.

Format Go changes with `gofmt`. Changes to the visual editor or Chromium integration also need `make browser-test`; Docker/Compose changes need `make docker-smoke`. `make check-containers` runs all container checks, including PostgreSQL. These checks require Docker and Compose 2.24.4 or newer. The Docker CLI must be accessible to the test process; pass `DOCKER='sudo docker'` to Make when needed. See [AGENTS.md](AGENTS.md) for the code map, behavior to preserve, and detailed validation guidance, and [browser testing](docs/browser.md) for Chromium checks and optional live-site trials.
